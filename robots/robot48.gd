...
func _init() -> void:
	des = [
		...
		COLOR_NAME[2] + '-' + COLOR_NAME[0] + '-' + COMPARE_NAME[0],
		COLOR_NAME[2] + '-' + COLOR_NAME[0] + '-' + COMPARE_NAME[1],
		COLOR_NAME[2] + '-' + COLOR_NAME[0] + '-' + COMPARE_NAME[2],
		'A specific number compare to another specific number',
		]
...
func put_answer(a: Answer) -> Array[int]:
	...
	var aa: = a.get_array()
	for i in aa.size():
		array.append(compare(aa[i], aa[i - 2]) + i * 3)
	return array
